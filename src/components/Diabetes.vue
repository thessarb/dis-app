<template>
  <div class="diabetes card">
    ...
    <section class="field">
      <label>Are you of Aboriginal, Torres Strait Islander, Pacific Islander or Maori descent? (required)</label>
      <div class="options">
        <label><input type="radio" v-model="form.descent" value="no" /> No (0 points)</label>
        <label><input type="radio" v-model="form.descent" value="yes" /> Yes (2 points)</label>
      </div>
    </section>
...
const allFilled = computed(() => {
  return (
    form.ageGroup && form.gender && form.descent !== '' && form.birthplace !== '' && form.historywithdiabetes !== '' &&
    form.glucosehistory !== '' && form.bpmedication !== '' && form.smoking !== '' &&
    form.vegfruit !== '' && form.physicalactivity !== '' && form.waistMeasurement !== null && form.waistMeasurement !== ''
  )
})
